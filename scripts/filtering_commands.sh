...
awk '{print $1 "\t" $2 "\t" $6 "\t" $36 "\t" $41}' "${local_dir}/BGE.VB.snp.tsv" > "${local_dir}/BGE.VB.snp.AORO.tsv"
...
# get AO and RO columns
awk '{print $1 "\t" $2 "\t" $6 "\t" $36 "\t" $41}' "${local_dir}/BGE.VB.snp.fil1.tsv" > "${local_dir}/BGE.VB.snp.fil1.AORO.tsv"
...
# get AO and RO columns
awk '{print $1 "\t" $2 "\t" $6 "\t" $36 "\t" $41}' "${local_dir}/BGE.VB.snp.fil2.tsv" > "${local_dir}/BGE.VB.snp.fil2.AORO.tsv"

# "After filtering, kept 3325944 out of a possible 3392758 Sites"
vcftools --vcf  "${local_dir}/BGE.VB.snp.fil2.vcf" --recode-INFO-all --out "${local_dir}/BGE.VB.snp.fil3" --recode 
# convert to TSV
vcf2tsv  "${local_dir}BGE.VB.snp.fil3.recode.vcf" > "${local_dir}/BGE.VB.FIL.tsv"
# get AO and RO columns
awk '{print $1 "\t" $2 "\t" $6 "\t" $36 "\t" $41}' "${local_dir}/BGE.VB.FIL.tsv" > "${local_dir}/BGE.VB.AORO.tsv"